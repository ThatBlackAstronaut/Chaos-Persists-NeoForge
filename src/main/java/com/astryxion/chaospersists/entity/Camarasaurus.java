...
public class Camarasaurus extends TamableAnimal {
    ...
    private boolean isGrazableBlock(Block block) {
        if (block == Blocks.VINE || block == Blocks.CACTUS) {
            return true;
        }
        ...
        return block.builtInRegistryHolder().is(BlockTags.LEAVES);
    }
    ...
    protected void customServerAiStep() {
        ...
        if (!this.isInSittingPose()
                && (this.getRandom().nextInt(20) == 0
                                && this.getCamarasaurusHealth() < this.mygetMaxHealth()
    ...
    @Override
    public Camarasaurus getBreedOffspring(ServerLevel level, AgeableMob partner) {
        return (Camarasaurus) this.getType().create(level);
    }
    ...
    public static boolean checkCamarasaurusSpawnRules(
            EntityType<Camarasaurus> type,
