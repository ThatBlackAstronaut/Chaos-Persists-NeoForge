...
public class StepAccross extends Item {
    ...
    public EnumActionResult onItemUse(EntityPlayer Player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        Block bid;
        for (int k = 1; k < length && (bid = world.getBlockState(new BlockPos(x + k * deltax, y - 1, z + k * deltaz)).getBlock()) == Blocks.AIR; ++k) {
            world.setBlockState(new BlockPos(x + k * deltax, y - 1, z + k * deltaz), Blocks.COBBLESTONE.getDefaultState(), 2);
            if ((k - 1) % 8 != 0 || (bid = world.getBlockState(new BlockPos(x + k * deltax, y, z + k * deltaz)).getBlock()) != Blocks.AIR) {
                continue;
            }
            world.setBlockState(new BlockPos(x + k * deltax, y, z + k * deltaz), ChaosPersists.ExtremeTorch.getDefaultState(), 2);
        }
