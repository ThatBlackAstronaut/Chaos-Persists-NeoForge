...
public class StepAccross extends Item {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        for (int k = 1;
                k < length
                        && (bid = world.getBlockState(new BlockPos(x + k * deltax, y - 1, z + k * deltaz)).getBlock())
                                == Blocks.AIR;
                ++k) {
            world.setBlock(
                    new BlockPos(x + k * deltax, y - 1, z + k * deltaz), Blocks.COBBLESTONE.defaultBlockState(), 2);
            if ((k - 1) % 8 != 0
                    || (bid = world.getBlockState(new BlockPos(x + k * deltax, y, z + k * deltaz)).getBlock())
                            != Blocks.AIR) {
                continue;
            }
            world.setBlock(
                    new BlockPos(x + k * deltax, y, z + k * deltaz),
